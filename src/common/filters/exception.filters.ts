...
export class AllExceptionsFilter implements ExceptionFilter {
	...
	private sendToSentry(
	...
	catch(exception: SupportedException, host: ArgumentsHost) {
		...
		// Handle AxiosError
			...
			errorMessage =
				exception.response?.data ?? exception.message ?? 'AXIOS_ERROR';
		}
		...
		const errorResponse = this.getSanitizedErrorResponse(
			status,
			errorMessage,
			request,
		);
